...
Клиент VPN-сервиса SAPN под Android. Приложение открывается сразу на экране
подключения: **вход не обязателен**. Без аккаунта работает всё, кроме нод сервиса —
свои VLESS-серверы (вставкой ссылки, сканом QR или импортом подписки), туннель,
per-app маршрутизация, direct-list, плитка, обновления. Вход добавляет онлайн-ноды
SAPN и подписку. Сырая VLESS-ссылка с серверов сервиса наружу не выдаётся: конфиг
приходит с control-plane по API и используется встроенным VPN-движком.
...
- **Работа без аккаунта** — приложение стартует на экране подключения, вход вынесен
  отдельной вкладкой и кнопкой в шапке. Гостю доступны свои серверы и весь остальной
  функционал; ноды SAPN и подписка появляются только после входа (у гостя бэкенд не
  дёргается вообще).
...
- **Выбор локации** из списка нод сервиса с пингом (RTT) и подключение через системный
  `VpnService`. Список нод виден только залогиненным.
...
app/src/main/java/ru/sapn/vpn/
  ...
  MainActivity.kt      # нижняя навигация; вкладка «Вход»/«Аккаунт» по состоянию сессии
